<div class="section-4">
    <!-- Ứng dụng Babi2School -->
    <div class="container">
        <h2 class="fancy-underline"> Dịch vụ Babi2School </h2>
        <div class="row py-5">
            <div class="col-md-6 col-12">
                <p class="text-dark"><b>Babi2School</b> là một dự án phát triển <b>ứng dụng</b> trên <b>thiết bị di động,
                        kết hợp với hệ thống xe 7 chỗ và giám sinh</b>, tạo thành một nền tảng hỗ trợ <b>phụ huynh, nhà
                        trường an toàn nhất hiện nay</b>, giúp phụ huynh giám sát, an tâm, tin tưởng trong việc <b>đưa
                        đón học sinh</b> trên xe 7 chỗ.</p>
                <a href="#" class="nav-link text-dark fst-italic">*Tải ứng dụng tại đây</a>
            </div>
            <div class="col-md-6 col-12"></div>
        </div>

        <div class="row mt-5">
            <div class="col-lg-6 col-md-6 mb-4 col-12 d-flex flex-column align-items-end">
                <x-link :href="route('home.index')">
                    <img src="{{ asset('public/assets/images/images-2.png') }}" height="128" width="372">
                </x-link>
                <x-link :href="route('home.index')">
                    <img src="{{ asset('public/assets/images/406f0f_c4ccc5a8f5d445408caf12050e202a82mv2.png') }}"
                        height="128" width="372">
                </x-link>

            </div>
            <div class="col-lg-6 col-md-6 col-12 mb-4">
                <img class=""
                    src="{{ asset('public/assets/images/Blue_Modern_Contact_Us_Instagram_Post__2_-removebg-preview.png') }}"
                    width="100%">
            </div>
        </div>
    </div>
    <div class="wave">
        <img src="{{ asset('public/assets/images/layered-waves-haikei.svg') }}" alt="">
    </div>
</div>
